...
We check this using the argument in Appendix B~\cite{full-ver}, which is obtained by compiling the
polynomial protocol for RAM in Appendix C~\cite{full-ver} into an argument of knowledge in the AGM.
...
showing $(c_S, c_o, c_S')\in \CLRAM$ (see Definition C.1~\cite{full-ver}). We provide the complete protocol
...
\begin{figure}[t!]
    \begin{mdframed}
        ...
        \begin{enumerate}[leftmargin=1em, label=\arabic*.]
        \item $\prover$ and $\verifier$ execute argument in Fig 11~\cite{full-ver} %~\ref{fig:covering-protocol}
