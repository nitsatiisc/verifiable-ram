...
$\wt{\tr}=(t(X),\op(X),A(X),V(X))$. Recall that we need to check two conditions on $\tr$, viz, (i) {\em monotonicity}:
...
The polynomial encodings in the previous section can be used to construct a polynomial protocol for
checking the membership in the language $\LRAM{I}{m}{m}$ for $m\in \N$. The polynomial protocol can be subsequently
compiled into a succinct argument using an extractable polynomial commitment scheme.
...
\end{enumerate}
We check above conditions over commitments. Let $\srs$ denote a $\kzg$ setup over a bilinear group, with
...
\begin{figure}[tb!]
    ...
    {\bf Prover's Input}: Transcripts $\tr, \tr^{*}$ and their polynomial encodings, permutation $\sigma$ such that $\tr^{*}=\sigma(\tr)$.
        \begin{enumerate}[leftmargin=1em, label=\arabic*]
            ...
            \end{itemize}
%            $f(X)=t(X) + \beta \op(X) + \beta^2 A(X) + \beta^3 V(X)$, $g(X) = t^\ast(X) + \beta \op^\ast(X)$
%            $+ \beta^2 A^\ast(X) + \beta^3 V^\ast(X)$. 
...
\begin{proof}
    ...
    \noindent{\bf Computing $a_i$. } Recall that for each $i \in \setind$, we have:
    ...
    \noindent{\bf Computing $p'(\xi^i)$.} We now show how to compute $p'(\xi^i)$ for each $i\in I$. Using the  product rule for derivatives, we have: \[p'(X)=q(X) \hat{Z}_K'(X)+q'(X) \hat{Z}_K(X)\]
...
%    Thus, it suffices to evaluate $\varphi_i=\Phi_i(\xi^i)$ efficiently for each  $i \in I$. 
...
%	Since $\eta_j$ is a (Lagrange) polynomial, 
...
%    This completes the proof for $a_i$ (Note that in the proof we throughout used that $|\setind| \leq |K|$ so that the various $O(|\setind|)$ operations do not contribute to the final complexity).
	...
	\noindent{\bf Modifications for Computing $b_j$ for $j\in K$.} For computing $b_j$, we proceed as in the case of $a_i$, with the roles of sets $I$ and $K$ swapped~(all of the corresponding lemmas can be modified accordingly). The only additional technical subtlety arises when we need to compute $\varphi_j=\Phi_j(\xi^j)$ for all $j \in K$, where the polynomial $\Phi_j(X)$ is defined as:
