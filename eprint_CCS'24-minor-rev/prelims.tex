...
\paragraph{\bf Lagrange Polynomials.}
...
We use the notation $b\gets \argoutput{\prover}{\verifier}{w}{x}$ to denote $\verifier$'s output in the interactive protocol involving $\prover$ and $\verifier$ with $w$ as $\prover$'s input and $x$ as the common input.
...
\paragraph{\bf Fiat-Shamir.} An interactive protocol is \emph{public-coin} if the verifier's messages are uniformly random strings. Public-coin protocols can be transformed into non-interactive arguments in the Random Oracle Model (ROM) by using the Fiat-Shamir~\cite{C:FiaSha86} heuristic to derive the verifier's messages as the output of a Random Oracle. 

\paragraph{\bf Modular Approach.}
...
\paragraph{\bf Algebraic Group Model.} 
...
\paragraph{\bf Real and Ideal Pairing Checks.} For an algebraic adversary $\Adv$ interacting in a protocol with a degree $Q$ SRS over a bilinear group, the verifier
...
\begin{itemize}[leftmargin=1em]
	...
	In the above equation, $a(X)=(X-\alpha_1)\cdots(X-\alpha_n)$, while $q(X)$ and $r(X)$ are the quotient and remainder polynomials when
	...
	\item $\KZGverify$ on input $(\srs,C,\vec{v}, \vec{\alpha}, \pi)$, outputs $1$ if the following equation is satisfied, and $0$ otherwise.
	\begin{align*}
...
\paragraph{\bf KZG for Vectors.} For $\vec{f}\in \F^N$, let $\enc{f}{\setN}$ denote the polynomial encoding of $\vec{f}$ over $\setN$ given by $\sum_{i=1}^N f_i\mu_i(X)$.
...
We define the {\em committed index lookup} relation $\RLOOK$ to consist of tuples of the form
