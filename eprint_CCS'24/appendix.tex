...
%				\end{align}
				\item $\prover$ computes the commitment $\gone{Q}$ = $\gone{Q(X)}$ and sends $\gone{Q}$ to $\verifier$.
				\item $\verifier$ samples $s\gets\F$ and sends $s$ to $\prover$.
				\item $\prover$ computes the evaluations $\val{s}{v}=v(s)$, $\val{\omega^m s}{v}=v(\omega^m s)$,
				$\val{\omega^{2m}s}{v}=v(\omega^{2m} s)$, $\val{s^3}{h}=h(s^3)$, $\val{s}{Q}=Q(s)$ and $\val{s}{Z}=Z(s)$, and sends $\val{s}{v}, \val{\omega^m s}{v}, \val{\omega^{2m}s}{v}, \val{s^3}{h}, \val{s}{Z}$ to $\verifier$.
				\item $\verifier$ samples $r\gets\F$ and sends $r$ to $\prover$.
				\item $\prover$ computes the following $\kzg$ proofs:
				\begin{enumerate}
					\item $\Pi_v=\kzgprove(\srs,v,(s,\omega^m s, \omega^{2m}s))$.
					\item $\Pi_h=\kzgprove(\srs,h,s^3)$.
					\item $\Pi_f=\kzgprove(\srs,f,s)$ where $f(X)=Z(X) + rQ(X)$.
				\end{enumerate}
				\item $\prover$ sends $\Pi_v$, $\Pi_h$ and $\Pi_f$ to $\verifier$.
				\item $\verifier$ computes commitments $\gone{h}$ and $\gone{f}$.\footnote{This can be done locally leveraging the linearity of the operation.}
				...
				\end{enumerate}
				\item $\verifier$ outputs accept if all the above checks succeed, else it outputs rejects.
			\end{enumerate}
		}
	\end{mdframed}
	%\vspace*{-5mm}
...
\begin{figure}[htbp]
    ...
    \begin{mdframed}
    {
        ...
        \begin{enumerate}[leftmargin=1em, label=\arabic*]
            \item $\prover$ computes sets $I_1, I_2$, and polynomials $Z_1(X), Z_2(X), \delta_T(X), \delta_A(X)$  as described in Section \ref{subsec:encoded-relations}.
            \item $\prover$ sends $\gone{Z_1(X)}$, $\gone{Z_2(X)}$, $\gone{\delta_T(X)}$, $\gone{\delta_A(X)}$ to $\verifier$.
            \item $\verifier$ samples $\gamma\gets \F$ and sends $\gamma$ to $\prover$.
            \item $\prover$ computes the following polynomials $(Q_1(X),Q_2(X))$ and its commitments $(\gone{Q_1(X)}$, $\gone{Q_2(X)})$:
            \begin{enumerate}
...
%            $$ \gamma^2 (\op(X) - 1)(V(\omega X) - V(X))] / Z_2(X) $$
            \item $\prover$ sends $\gone{Q_1(X)}$, $\gone{Q_2(X)}$ to $\verifier$.
            \item $\verifier$ samples $s\gets\F$ and sends $s$ to $\verifier$.
            \item $\prover$ computes the evaluations $\val{s}{A}=A(s)$, $\val{\omega s}{A}=A(\omega s)$, $\val{s}{\delta_A} = \delta_A(s)$, $\val{s}{t}=t(s)$, $\val{\omega s}{t}=t(\omega s)$, $\val{s}{\delta_T} = \delta_T(s)$, $\val{s}{\op}=\op(s)$, $\val{s}{V}=V(s)$, $\val{\omega s}{V}=V(\omega s)$, $\val{s}{Q_1}=Q_1(s)$, $\val{s}{Q_2}=Q_2(s)$, $\val{s}{Z_1}=Z_1(s)$, $\val{s}{Z_2}=Z_2(s)$, and sends $\val{s}{A}$, $\val{\omega s}{A}$, $\val{s}{\delta_A}$, $\val{s}{t}$, $\val{\omega s}{t}$, $\val{s}{\delta_T}$, $\val{s}{\op}$, $\val{s}{V}$, $\val{\omega s}{V}$, $\val{s}{Q_1}$, $\val{s}{Q_2}$, $\val{s}{Z_1}$ and $\val{s}{Z_2}$ to $\verifier$.
            ...
            \end{enumerate}
            \item $\verifier$ samples $r_1, r_2\gets\F$ and sends $r_1,r_2$ to $\prover$.
            \item $\prover$ computes the following polynomials and $\kzg$ proofs:
            \begin{enumerate}
            ...
            \end{enumerate}
            \item $\prover$ sends $\Pi_{\omega s}, \Pi_{s}$ to $\verifier$.
            \item $\verifier$ computes the following commitments and evaluations: \footnote{This is done locally by leveraging the linearity of the operation.}
            \begin{enumerate}
            ...
            \end{enumerate}
            \moumita{ This notation is not defined anywhere, so removed it.
            ...
            \item $\left<\prover, \verifier\right>$ runs the subvector lookup protocol (eg. using \cite{EPRINT:EagFioGab22}) to prove the following instances:\\
             $A\subset[N]$, $t\subset[N]$, $\delta_A\subset[N]$, and $\delta_T\subset[N]$.
            \item $\verifier$ outputs accept if all the above checks succeed (including checks for lookup protocols), else it outputs reject.
        \end{enumerate}
    }
    \end{mdframed}
...
\begin{figure}[htbp]
    ...
    \begin{mdframed}
    {
        ...
        $Z(X)=\prod_{i=1}^m (X-\omega^i)$.\\
		    {\bf Prover's Input}: $\tr, T, \vec{o}, T'$ and their polynomial encodings.
        \begin{enumerate}
	            \item $\verifier$ samples $\beta,\gamma\gets \F$ and sends $\beta,\gamma$ to $\verifier$.
...
%            \end{align}
            \item $\prover$ computes commitment $\gone{Q}$ to $Q(X)$ and sends $\gone{Q}$ to $\verifier$.
            \item $\verifier$ samples $s\gets\F$ and sends $s$ to $\prover$.
            \item $\prover$ computes evaluations $\val{s}{G}=G(s)$, $\val{\omega^m s}{G}=G(\omega^m s)$,
            $\val{\omega^{2m}s}{G}=G(\omega^{2m} s)$, $\val{s^3}{H}=H(s^3)$, $\val{s}{Q}=Q(s)$ and $\val{s}{Z}=Z(s)$, and 
            sends $\val{s}{G}$, $\val{\omega^m s}{G}$, $\val{\omega^{2m}s}{G}$, $\val{s^3}{H}$, $\val{s}{Q}$, and $\val{s}{Z}$ to $\verifier$.
            \item $\verifier$ samples $r\gets\F$ and sends $r$ to $\prover$.
            \item $\prover$ computes the following $\kzg$ proofs:
            \begin{enumerate}
            ...
            \end{enumerate}
            \item $\prover$ sends $\Pi_G, \Pi_H, \Pi_F$ to $\verifier$.
            \item $\verifier$ computes $\gone{G(X)},\gone{H(X)}$ and $\gone{F(X)}$. \footnote{This can be done locally leveraging the linearity of the operation.}
            \item $\verifier$ checks the following:
            \begin{enumerate}
...
\begin{figure}[htbp]
    ...
    {\bf Prover's Input}: Transcripts $\tr, \tr^{*}$ and their polynomial encodings, permutation $\sigma$ such that $\tr^{*}=\sigma(\tr)$.
        \begin{enumerate}[leftmargin=1em, label=\arabic*]
            \item $\verifier$ samples $\alpha,\beta, \chi\gets \F$ and sends $\alpha,\beta$ to $\prover$.
            \item $\prover$ computes the following polynomials:
            \begin{enumerate}
            	\item $f(X)=t(X) + \beta \op(X) + \beta^2 A(X) + \beta^3 V(X)$
            	\item $g(X) = t^\ast(X) + \beta \op^\ast(X)+ \beta^2 A^\ast(X) + \beta^3 V^\ast(X)$
            \end{enumerate}
            \item $\prover$ computes polynomials $z(X),q(X)$ as:
            \begin{enumerate}
            ...
            \end{enumerate}
            \item $\prover$ computes commitments $\gone{z(X)}, \gone{q(X)}$ to polynomials $z(X)$ and $q(X)$ respectively, and sends $\gone{z(X)}$, $\gone{q(X)}$ to $\verifier$.
            \item $\verifier$ computes commitments $[f]_1, [g]_1$. \footnote{This can be done locally leveraging the linearity of the operation.}
...
The range checks in (C7) can be checked using polynomial protocols in sub-vector lookup arguments such as Caulk+~\cite{EPRINT:PosKat22}, CQ~\cite{EPRINT:EagFioGab22}. The protocol(compiled using KZG commitments in AGM) can be found in Figure \ref{fig:encoded-relations}. The overall covering protocol for $\CLRAM$ which combines Figures \ref{fig:time-ordered-transcript}, \ref{fig:permutated-transcripts}, \ref{fig:encoded-relations} in this order can be found in Figure \ref{fig:covering-protocol}.
\begin{figure}[htbp]

    \begin{mdframed}
    {
            {\bf Common Input}: Commitments $c_T=(c_a,c_v)$, $c_o=(\bar{c}_\op, \bar{c}_a, \bar{c}_v)$, $c_T'=(c_a', c_v')$ to $ T, T'$ and $ \vec{o}$ respectively.
            {\bf Prover's Input}: $ T, T', \vec{o}$ and their polynomial encodings.
        \begin{enumerate}
            \item $\prover$ computes the following:
            \begin{enumerate}
                \item $\tr$ (time ordered transcript corresponding to $T, \vec{o}, T'$), its polynomial encoding, and its commitment $c_\tr=(c_t, c_\op, c_A, c_V)$ .
%                \item $c_\tr=(c_t, c_\op, c_A, c_V)$ which is the commitment of $\tr$.
                \item $Z(X)=\prod_{i=1}^m (X-\omega^i)$ and its commitment $[Z(X)]_1$.
            \end{enumerate}
            \item $\prover$ sends $c_\tr=(c_t, c_\op, c_A, c_V)$ and $[Z(X)]_1$ to $\verifier$.
            \item $\left<\prover, \verifier \right>$ run the protocol for checking correctness of time ordered transcript (Figure \ref{fig:time-ordered-transcript}).
            ...
            \item $\prover$ computes the commitment $c_\tr^\ast=(c_t^\ast,c_\op^\ast, c_A^\ast, c_V^\ast)$ of $\tr^{*}$, and sends $c_\tr^\ast$ to $\verifier$.
            \item $\left<\prover, \verifier \right>$ run the protocol for checking that the two transcripts are permutations of each other (Figure \ref{fig:permutated-transcripts}).
            \item $\left<\prover, \verifier \right>$ run the protocol for checking the constraints given in Lemma \ref{lem:addr-ordered-transcript} (Figure \ref{fig:encoded-relations}).
...
\begin{figure}[t!]
    \begin{mdframed}
        ...
        \begin{enumerate}[leftmargin=1em, label=\arabic*.]
            ...
            $\val{h(\alpha)}{g}=g(h(\alpha))$, $\val{\alpha}{Q}=Q(\alpha)$, $\val{\alpha}{q}=q(\alpha)$, $\val{\alpha}{Z}=Z_I(\alpha)$, and sends $\val{\alpha}{v}$, $\val{\alpha}{a}$, $\val{\alpha}{h}$, $\val{\alpha}{K}$, $\val{h(\alpha)}{g}$, $\val{\alpha}{Q}$, $\val{\alpha}{q}$, $\val{\alpha}{Z}$ to $\verifier$.
            \item $\prover$ computes polynomial $u(X)$ as in Section~\ref{subsec:proximity-ram}.
            \item $\prover$ computes $c_u=\gone{u(X)}$ and sends $c_u$ to $\verifier$.
            \item $\verifier$ checks $\val{\alpha}{Q}(\alpha^m-1)=\val{h(\alpha)}{g}-\val{\alpha}{v}-\gamma \val{\alpha}{a}$ and $\val{\alpha}{Z}\val{\alpha}{q}=\val{\alpha}{K}$.
            \item $\verifier$ sets $\beta$ as $\beta=\val{\alpha}{K}-1$.
            \item $\verifier$ samples $\theta\gets\F$ and sends $\theta$ to $\prover$.
        \end{enumerate}

        {\bf Round 5}: Check correctness of $K$.
        \begin{enumerate}[leftmargin=1em, label=\arabic*.]
            \item $\prover$ computes $ Q'(X) = \big(u(\nu X)(1+\beta \tau_1(X))-u(X)(\alpha - h(X)) \quad + \theta \tau_1(X)(u(X)-1)\big)/Z_\setV(X)$.
            \item $\prover$ computes $c_Q'=\gone{Q'(X)}$ and sends $c_Q'$ to verifier.
            \item $\verifier$ samples $x\gets \F$ and sends $x$ to $\prover$.
        \end{enumerate}

        {\bf Round 6}: Prover sends more evaluations.
        \begin{enumerate}[leftmargin=1em, label=\arabic*.]
            \item $\prover$ computes $\val{x}{u}=u(x)$, $\val{\nu x}{u}=u(\nu x)$, $\val{x}{h}=h(x)$, $\val{x}{Q'}=Q'(x)$ and sends $\val{x}{u}$, $\val{\nu x}{u}$, $\val{x}{h}$, $\val{x}{Q'}$ to $\verifier$.
            \item $\verifier$ checks $\val{x}{Q'}(x^m-1)=\val{\nu x}{u}(1+\beta \tau_1(x))-\val{x}{u}(\alpha - \val{x}{h})$.
            \item $\verifier$ samples $r_a, r_h, r_q, r_v, r_K, r_Q, r_Z\gets \F$ as well as $r_h', r_u', r_Q'\gets \F$, and sends $r_a, r_h, r_q, r_v, r_K, r_Q, r_Z,r_h', r_u', r_Q'$ to $\prover$.
        \end{enumerate}
...
%        \end{center}
        ...
        \begin{enumerate}[leftmargin=1em, label=\arabic*.]
            \item $\prover$ computes the following polynomials:
            \begin{enumerate}
            	\item $\Phi_\alpha(X) = r_a a(X)+ r_h h(X) + r_q q(X) + r_v v(X) \quad + r_K K(X) + r_Q Q(X) + r_Z Z_I(X)$.
            	\item $\Phi_x(X) = r_h' h(X) + r_u' u(X)+r_Q'Q'(X)$.
            \end{enumerate}
%            \begin{align*}
%                \Phi_\alpha(X) &= r_a a(X)+ r_h h(X) + r_q q(X) + r_v v(X) \quad + r_K K(X) + r_Q Q(X) + r_Z Z_I(X) \\
%                \Phi_x(X) &= r_h' h(X) + r_u' u(X)+r_Q'Q'(X)
%            \end{align*}
            \item $\prover$ computes the following $\kzg$ proofs:
            		\begin{enumerate}
            			\item $\Pi_\alpha = \KZGopen(\srs, \Phi_\alpha(X), \alpha)$.
            			\item $\Pi_x = \KZGopen(\srs, \Phi_x(X), x)$.
            			\item $\Pi_g = \KZGopen(\srs, g(X), \val{\alpha}{h})$.
            			\item $\Pi_u = \KZGopen(\srs, u(X), \nu x)$.
            		\end{enumerate}
...
%            \item $\prover$ computes $\Pi_u = \KZGopen(\srs, u(X), \nu x)$.
            \item $\prover$ sends $\Pi_\alpha$, $\Pi_x$, $\Pi_g$ and $\Pi_u$ to $\verifier$.
            \item $\verifier$ computes the following commitments and evalutations: \footnote{This is done locally by leveraging the linearity of the operation.}
            \begin{enumerate}
            	\item $\gone{\Phi_\alpha} = r_a c_a + r_h c_h + r_q c_q + r_v c_v + r_z c_z + r_K c_K + r_Q c_Q$.
            	\item $\gone{\Phi_x} = r_h' c_h + r_u' c_u + r_Q' c_Q'$.
            	\item $V_{\alpha} = r_a \val{\alpha}{a} + r_h \val{\alpha}{h} + r_q \val{\alpha}{q} + r_v \val{\alpha}{v} \quad + r_z\val{\alpha}{Z} r_K \val{\alpha}{K} + r_Q \val{\alpha}{Q}$
            	\item $V_x = r_h' \val{x}{h} + r_u' \val{x}{u}+r_Q' \val{x}{Q'}$
            \end{enumerate}
%            \begin{align*}
%                \gone{\Phi_\alpha} &= r_a c_a + r_h c_h + r_q c_q + r_v c_v + r_z c_z + r_K c_K + r_Q c_Q. \\
%                \gone{\Phi_x} &= r_h' c_h + r_u' c_u + r_Q' c_Q' \\
%                V_{\alpha} &= r_a \val{\alpha}{a} + r_h \val{\alpha}{h} + r_q \val{\alpha}{q} + r_v \val{\alpha}{v} \quad + r_z\val{\alpha}{Z} r_K \val{\alpha}{K} + r_Q \val{\alpha}{Q}. \\
...
%            \end{align*}
            ...
            \item $\prover$ and $\verifier$ execute argument for $(c_S, c_o, c_S')\in \CLRAM$. \footnote{Argument for $(c_S, c_o, c_S')\in \CLRAM$ is executed as per Section~\ref{sec:poly-proto-ram}.}
        \end{enumerate}
    \end{mdframed}
    \caption{Batching-Efficient RAM Protocol-Continued}
    \label{fig:complete-listing-2}
