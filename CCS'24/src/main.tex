...
%	\renewcommand{\shortauthors}{Trovato et al.}
	...
	\begin{abstract}
		...
		proving a batch of $m$ updates on a RAM of size $N$, should incur a cost sub-linear in $N$.
		...
		Recent works have explored cryptographic accumulators based on unknown-order groups (RSA, class-groups)
		to model the RAM state. While the recent RSA accumulator based approaches
		offer significant improvement over classical methods,
		they continue to incur linear overhead in the size of the
		accumulated set to compute witnesses, as well as concretely prohibitive constant overhead.
		
		In this paper, we realize a batching-efficient RAM with superior asymptotic and concrete
		costs. Towards this (i) we build on recent constructions of lookup arguments to allow efficient
		lookups even in presence of table updates, and (ii) we realize a variant of sub-vector relation
		...
		for a batch of $m$ updates on the RAM of size $N$.
		...
		This is we believe to be of independent interest.
