...
with access to pre-computed parameters for an ``approximate'' table. In particular, we show that lookup protocols in
...
$\setN_I=\{{h(\nu^i)}: i\in I\}=\{\xi^{a_i}:i\in I\}\subseteq \setN$, where $I=\{a_i: i\in [m]\}$. The prover
...
\subsection{Almost idential RAM States}\label{subsec:proximity-ram}
...
$\{\xi^i: i\in I\}$. Thus, Equation \eqref{eq:poly-q} implies that $Q$ is at most degree $|I|-1$ polynomial, with
$Q(\xi^i)=\Delta_i Z_I'(\xi^i)/\vpolyN'(\xi^i)$ for $i\in I$.
The prover can therefore interpolate $Q(X)$ (in power basis)
in $O(|I|\log^2 |I|)$ $\F$-operations and compute $\gone{Q(X)}$ in $O(|I|)$ $\Gone$-operations. The prover sends the
commitment $\gone{Q(X)}$ to the verifier. Finally, the verifier can
check the identity $Z_I(T - T') = Q\cdot Z_\setN$ at a random point $\alpha\gets \F$, as it knows commitments to all
...
listing in Figure ~\ref{fig:complete-listing}.
...
\begin{mdframed}
    ...
    \begin{enumerate}[leftmargin=1em, label=\arabic*.]
        ...
        \item $\prover$ sends $c_p = \gone{P(X)}$, $c_z=\gone{Z_I(X)}$, $c_h=\gone{h(X)}$, $c_l = \gone{\ell_I(X)}$,
        $c_k = \gone{K(X)}$, $c_q = \gone{q(X)}$.
    ...
    \begin{enumerate}[leftmargin=1em, label=\arabic*.]
        ...
        \item $\prover$ computes: $W = \sum_{i\in [m]} (W_1^i + \chi {W_1^i}' + \gamma W_2^i + \gamma^2 W_3^i)$.
        ...
        \item $\verifier$ checks: $e(C_G - c_g, \gone{1})=e(c_z, W)$.
    ...
    \begin{enumerate}[leftmargin=1em, label=\arabic*.]
        ...
        \item $\prover$ sends $\val{\alpha}{v}$, $\val{\alpha}{a}$, $\val{\alpha}{h}$, $\val{\alpha}{K}$, $\val{h(\alpha)}{g}$, $\val{\alpha}{Q_2}$,
...
\begin{figure}[t!]
    ...
    {\bf Round 6}: Prover sends more evaluations.
        \begin{enumerate}[leftmargin=1em, label=\arabic*.]
            ...
            \item $\verifier$ sends $r_a, r_h, r_q, r_v, r_K, r_Q, r_Z\gets \F$ and $r_h', r_u'\gets \F$.
        \end{enumerate}
