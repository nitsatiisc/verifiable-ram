...
check the identity $Z_I(T - T^*) = D\cdot Z_\setN$ \moumita{$Z_I(T - T') = D\cdot Z_\setN$} by a pairing check. For this, since the tables are committed in $\Gone$, prover will need to send $\elttwo{Z_I(X)}$.
