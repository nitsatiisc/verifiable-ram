...
\moumita{To proceed, the honest prover commits to polynomial $Z_I(X)$ and proves (i) $Z_I(X)\cdot (T(X) - T^*(X)) = 0 \text{ mod } Z_\setN$ and (ii) the zeroes
	of $Z_I(X)$ form a subset of zeroes of $\setN_I(X)$ as defined.}
...
check the identity $Z_I(T - T^*) = D\cdot Z_\setN$ \moumita{$Z_I(T - T') = D\cdot Z_\setN$ [OR] $Z_I(X) \cdot  (T(X) - T^*(X)) = D(X) \cdot Z_\setN(X)$ } by a pairing check. For this, since the tables are committed in $\Gone$, prover will need to send $\elttwo{Z_I(X)}$.
