...
\subsubsection{Committed Index Lookup}
...
\subsubsection{Fast Lookup from Approximate Setup}
We build upon the rich body of work on polynomial protocols enabling efficient lookups from static tables
~\cite{CCS:ZBKMNS22,EPRINT:PosKat22,EPRINT:ZGKMR22,EPRINT:EagFioGab22}, which rely on expensive table-dependent pre-computation
to optimise online proving performance. We make the first attempt towards breaking this rigid dependence.
Our key idea is to extend the utiltiy of pre-computed
...
\subsubsection{Localizing changes in RAMs} While the above two components allow us to reliably extract sub-RAMs
...
\subsubsection{Polynomial Protocol for Memory Checking} To complete the verification, we need to show that
...
\subsubsection{Efficiency} We conclude the overview with a discussion of efficiency achieved by our scheme, and
...
\subsubsection{Liveliness} To maintain {\em liveliness} of the system in scenarios such as rollups, we must carefully
...
is live at all times. Clearly, a faster offline pre-computation using parallel implemenation would allow us to stay
